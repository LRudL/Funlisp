...
(defn eval-if
  ...
  (if (first (eval-exp predicate env))
...
(defn eval-exp-list
  ...
  on to the remaining expressions in the sequence. Returns nil if passed an
...
(defn eval-exp
  ...
  (cond
    ...
    ; the quote function:
    ; (a Lisp-specific thing to return without evaluating, e.g. (quote x) = x)
    (starts-with? exp 'quote) [(second exp) env]
    ; if:
    ...
    [(conj (eval-exp (third exp) env)  ; note Clojure cons argument order
           (eval-exp (second exp) env)); (first list, then thing to add)
     env]
    ; print
    ...
    [(function (rest (rest exp)) ; function body, ((+ x y)) above
     ...
     env]
    ; function application (tried if none of the above match)
    true [(apply-fn
           (result (first exp) env) ;assume first thing is a function structure
