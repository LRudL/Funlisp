...
(defn args-parse
  "Parses argument lists of the form [arg1 arg2 ... argn] or
  [arg1 arg2 & rest-args], where the latter is split on & into
  named args and a name, rest-args, for the rest of the argument list,
  as well as a getter function that can be applied to the full argument list
  to get only the ones that 'overflow' into rest-args."
  [args]
  (let [split-args (partition-by #{'&} args)
        ; the above does, e.g., (a1 a2 & as) -> ((a1 a2) (&) (as))
        named-args (if (starts-with? (first split-args) '&)
                     '() ; for the case that the function has no named args
                     (first split-args))]
    (if (empty? (rest split-args))
      {:arg-names named-args
       :rest-args-name nil
       :rest-args-getter nil}
      {:arg-names named-args
       :rest-args-name (first (last split-args))
       :rest-args-getter (partial drop (count named-args))})))

(defn match-args
  "Takes a parse of args as generated by args-parse and a list of argument
  values, and returns a map associating argument names with values.
  Example: arg-parse = (args-parse (a b & c)), arg-vals = [1 2 3 4 5] gives
  {a 1, b 2, c (3 4 5)}"
  [arg-parse arg-vals]
  (merge ; zipmap goes as far as the min length list has items:
         (zipmap (arg-parse :arg-names) arg-vals) ; map from key list, value list
         (if (nil? (arg-parse :rest-args-name))
           {}
           {(arg-parse :rest-args-name) ((arg-parse :rest-args-getter) arg-vals)})))
...
(defn recursive-replace
  ...
  that's a key of map rmap with the value it maps to in rmap. Used for
  implementing (unhygienic) macros."
  [rmap coll]
...
(defn macro-expand
  ...
  [macro args]
  (recursive-replace (match-args (macro :args) args)
                     (macro :body)))
